...
from bosdyn.client.frame_helpers import get_odom_tform_body, ODOM_FRAME_NAME, BODY_FRAME_NAME
...
class SpotWrapper():
    ...
    def __init__(self, username, password, hostname, logger, estop_timeout=9.0, rates = {}, callbacks = {}):
        ...
        if self._robot:
            ...
            self._point_cloud_task = AsyncPointCloudService(self._point_cloud_client, self._logger, max(0.0, self._rates.get("point_cloud", 0.0)), self._callbacks.get("lidar_points", lambda:None), self._point_cloud_requests)
            ...
            self._async_tasks = AsyncTasks([self._robot_state_task, 
                    ...
                    self._rear_image_task, self._idle_task, self._estop_monitor, self._point_cloud_task])

...
def get_default_camera_info_msg():
...
def getImageMsg(data, spot_wrapper):
    ...
    for frame_name in data.shot.transforms_snapshot.child_to_parent_edge_map:
        if data.shot.transforms_snapshot.child_to_parent_edge_map.get(frame_name).parent_frame_name:
            ...
            new_tf.header.stamp.sec = local_time.seconds
            new_tf.header.stamp.nanosec = local_time.nanos
    ...
    image_msg.header.stamp.sec = local_time.seconds
    image_msg.header.stamp.nanosec = local_time.nanos
    ...
    camera_info_msg = get_default_camera_info_msg()
    local_time = spot_wrapper.robotToLocalTime(data.shot.acquisition_time)

    camera_info_msg.header.stamp = Time()
    camera_info_msg.header.stamp.sec = local_time.seconds
    camera_info_msg.header.stamp.nanosec = local_time.nanos
...
def GetPointCloudMsgFromGraphNav(graph_nav, spot_wrapper):
    """Return a point cloud from the graph nav data"""

    # Decode the points
    number_of_points = graph_nav.live_data.point_cloud.num_points
    mask = f'<{number_of_points * 3}f'
    points = unpack(
        mask,
        graph_nav.live_data.point_cloud.data,
    )
    ...
    point_cloud_msg.header.stamp.sec = \
        graph_nav.live_data.point_cloud.source.acquisition_time.seconds
    point_cloud_msg.header.stamp.nanosec = \
        graph_nav.live_data.point_cloud.source.acquisition_time.nanos
    ...
    for point in iter_points:
        p = Point32()
        p.x = point
        p.y = next(iter_points)
        p.z = next(iter_points)
...
def GetJointStatesFromState(state, spot_wrapper):
    ...
    joint_state.header.stamp.sec = local_time.seconds
    joint_state.header.stamp.nanosec = local_time.nanos
...
def GetEStopStateFromState(state, spot_wrapper):
    ...
    for estop in state.estop_states:
        ...
        estop_msg.header.stamp = Time()
        estop_msg.header.stamp.sec = local_time.seconds
        estop_msg.header.stamp.nanosec = local_time.nanos
...
def GetVisionTwistFromState(state, spot_wrapper):
    """Maps vision data from robot state proto to ROS TwistWithCovarianceStamped message
    ...
    twist_odom_msg.header.stamp = Time()
    twist_odom_msg.header.stamp.sec = local_time.seconds
    twist_odom_msg.header.stamp.nanosec = local_time.nanos

    twist_odom_msg.twist.twist.linear.x = \
        state.kinematic_state.velocity_of_body_in_vision.linear.x
    twist_odom_msg.twist.twist.linear.y = \
        state.kinematic_state.velocity_of_body_in_vision.linear.y
    twist_odom_msg.twist.twist.linear.z = \
        state.kinematic_state.velocity_of_body_in_vision.linear.z
    twist_odom_msg.twist.twist.angular.x = \
        state.kinematic_state.velocity_of_body_in_vision.angular.x
    twist_odom_msg.twist.twist.angular.y = \
        state.kinematic_state.velocity_of_body_in_vision.angular.y
    twist_odom_msg.twist.twist.angular.z = \
        state.kinematic_state.velocity_of_body_in_vision.angular.z
    ...
    for frame_name in state.kinematic_state.transforms_snapshot.child_to_parent_edge_map:
        if state.kinematic_state.transforms_snapshot.child_to_parent_edge_map.get(frame_name).parent_frame_name:
            ...
            new_tf.transform.translation.x = \
                transform.parent_tform_child.position.x
            new_tf.transform.translation.y = \
                transform.parent_tform_child.position.y
            new_tf.transform.translation.z = \
                transform.parent_tform_child.position.z
            new_tf.transform.rotation.x = \
                transform.parent_tform_child.rotation.x
            new_tf.transform.rotation.y = \
                transform.parent_tform_child.rotation.y
            new_tf.transform.rotation.z = \
                transform.parent_tform_child.rotation.z
            new_tf.transform.rotation.w = \
                transform.parent_tform_child.rotation.w
            tf_msg.transforms.append(new_tf)
...
def GetBatteryStatesFromState(state, spot_wrapper):
    ...
    for battery in state.battery_states:
        ...
        battery_msg.header.stamp.sec = local_time.seconds
        battery_msg.header.stamp.nanosec = local_time.nanos
        ...
        battery_msg.estimated_runtime.sec = battery.estimated_runtime.seconds
        battery_msg.estimated_runtime.nanosec = battery.estimated_runtime.nanos
...
def GetPowerStatesFromState(state, spot_wrapper):
    ...
    power_state_msg.header.stamp = Time()
    power_state_msg.header.stamp.sec = local_time.seconds
    power_state_msg.header.stamp.nanosec = local_time.nanos
    ...
    power_state_msg.locomotion_estimated_runtime.sec = \
        state.power_state.locomotion_estimated_runtime.seconds
    power_state_msg.locomotion_estimated_runtime.nanosec = \
...
def getBehaviorFaults(behavior_faults, spot_wrapper):
    ...
    for fault in behavior_faults:
        ...
        new_fault.header.stamp = Time()
        new_fault.header.stamp.sec = local_time.seconds
        new_fault.header.stamp.nanosec = local_time.nanos
...
def getSystemFaults(system_faults, spot_wrapper):
    ...
    for fault in system_faults:
        ...
        new_fault.duration = Duration()
        new_fault.duration.sec = local_time.seconds
        new_fault.duration.nanosec = local_time.nanos
